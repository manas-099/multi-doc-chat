...
import uuid
from datetime import datetime
from MultiDocChat.utils.file_io import save_uploaded_files
from MultiDocChat.utils.document_ops import load_documents
import hashlib
import sys
...
def generatesessionid()->str:
    ...
    return f"session_{timestamp}_{uniq_id}"

class ChatIngestor:
    def __init__(self,
                 temp_base_file:str="data",
                 faiss_base_file:str="faiss_base",
        ...
        try:
            ...
            self.session_id=session_id or generatesessionid()
            self.temp_bass_file=Path(temp_base_file); self.temp_bass_file.mkdir(parents=True, exist_ok=True)
            self.faiss_base_file=Path(faiss_base_file); self.faiss_base_file.mkdir(parents=True,exist_ok=True)
            self.temp_dir = self._resolve_dir(self.temp_bass_file)
            self.faiss_dir = self._resolve_dir(self.faiss_base_file)
